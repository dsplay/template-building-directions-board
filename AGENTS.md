...
The DSPLAY **Building Directions Board** template — a [React](https://reactjs.org/) app built with [Vite](https://vitejs.dev/), showing a paginated directory of destinations within a building. Requires Node.js 22.22.2+, 24.15.0+, or 26+ (see `.nvmrc`). See README.md for the media data shape.
...
src/
  ...
  components/
    app/                      <-- top-level component (loader, i18n, pagination)
    main/                     <-- top logo bar
    secondary/                <-- one row per destination
...
`package.json`'s `"name"` must identify this template, not the boilerplate it was cloned from — see [`template-boilerplate-react`](https://github.com/dsplay/template-boilerplate-react)'s AGENTS.md for the full convention. This template's is `dsplay-template-building-directions-board` (it was previously `template-elevator`, a leftover from an earlier working name).
...
4. **Template variables** — a `Key | Type | Default | Description` table, ending with the "register as Template Vars in the DSPLAY CMS" reminder. This template has none — see "Runtime model" below.
...
- **Every static, developer-authored piece of UI text must go through `react-i18next`'s `t()`** — never a hardcoded string in JSX. This template's only two static strings are the `alt` text on the main logo and the direction arrow ("Logo"/"Direction") — everything else on screen (`name`/`place`/`floor`) is `media.targets` content, not developer-authored text.
...
- This template's `src/i18n.js` previously existed but was entirely dead — no component imported it, called `useTranslation()`, or rendered `t()` anywhere, and it imported `date-fns` locale objects and `i18next`/`i18next-browser-languagedetector` that weren't even declared as dependencies (silently never triggered since nothing imported the file). It's now rewritten and actually wired up via `I18nextProvider` in `app/index.jsx`.
- The pre-existing `alt="Direção"` on the destination's own logo image (as opposed to the direction arrow) looked like a copy-paste mistake — both images shared the same Portuguese hardcoded alt text, but only the arrow image is actually about direction. Fixed to use the `Logo`/`Direction` keys respectively, matching what each image actually shows.

## Runtime model

- `public/dsplay-data.js` defines `dsplay_config`/`dsplay_media` mock globals used only in **development** (no `dsplay_template` — this template reads everything from `media`, not Template Vars). `build.sh` blanks its content in the production build — the DSPLAY Android app injects the real `window.DSPLAY.getData()` before any script runs.
- [`@dsplay/react-template-utils`](https://github.com/dsplay/react-template-utils) exposes `useMedia`/`useScreenInfo`/`useConfig`/`Loader`.
...
- **New `dsplay_template` variable keys should use `snake_case`** (e.g. `background_color`, not `backgroundColor`) — the DSPLAY CMS Manager auto-generates each variable's on-screen label from its key name, and snake_case reads more naturally there. This template currently has none (it reads from `media` instead — see above), but this applies if any are ever added, and never applies retroactively to rename existing keys of other templates just to match, since they're already registered/in use in production CMS configurations.
- `src/components/app/index.jsx` paginates `media.targets` based on the screen's height, cycling through pages on a timer derived from `media.duration` (capped by `media.maxPageDurationSeconds`).

## Template variable manifest

`vite.config.js` registers `@dsplay/template-manifest`'s Vite plugin, which on every build statically scans `src/` for `tval`/`useTemplateVal`-style reads and captures `public/dsplay-data.js` as example data, writing `template-variables.json` + `template-example-data.json` into the build output — and therefore into `template.zip` (`npm run zip` runs `build.sh`, which zips the whole build output). Since this template has no `dsplay_template` variables, `template-variables.json` legitimately lists zero variables — that's expected, not a scanner bug. See [@dsplay/template-manifest](https://www.npmjs.com/package/@dsplay/template-manifest) for exactly what it detects.
